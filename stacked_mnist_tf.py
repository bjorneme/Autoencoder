...
class DataMode(Enum):
    ...
    """
    MONO_BINARY_COMPLETE: Standard one-channel MNIST dataset. All classes
    represented. Binarized. Use for learning standard generative models, check
    coverage, etc.
    """
    MONO_BINARY_COMPLETE = auto()

    """
    MONO_BINARY_MISSING: Standard one-channel MNIST dataset, but one class taken
    out. Use for testing "anomaly detection". Binarized.
    """
    MONO_BINARY_MISSING = auto()

    """
    MONO_FLOAT_COMPLETE: Standard one-channel MNIST dataset, All classes there.
    Use for testing coverage etc. Data represented by their float values (not
    ...
    """
    MONO_FLOAT_MISSING: Standard one-channel MNIST dataset, but one class taken
    out. Use for testing anomaly detection use-case. Data represented by their
    float values (not binarized). Can be easier to learn, but does not give as
    easy a probabilistic understanding.
    """
    MONO_FLOAT_MISSING = auto()

    """
    COLOR_<WHATEVER>: These are *STACKED* versions of MNIST, i.e., three color
    channels with one digit in each channel. Subgroups
    [BINARY|FLOAT]_[COMPLETE|MISSING]: As above for the MONO versions.
    """
...
class StackedMNISTData:
    ...
    def __init__(self, mode: DataMode, default_batch_size: int = 256) -> None:
        ...
        self.train_images, self.train_labels = self._prepare_data_set(training=True)
        self.test_images, self.test_labels = self._prepare_data_set(training=False)
    ...
    def _prepare_data_set(self, training: bool = True) -> tuple:
        """
        ...
        if self.channels > 1:
            ...
            # Choose the images to get a thing that is <default_batch_size, 28,
            # 28, self.channels> where the last dim is over the dims of the
            # indexes
            ...
            for channel in range(self.channels):
                ...
                generated_labels += (
                    np.power(10, self.channels - channel) * labels[indexes[:, channel]]
                )
    ...
    def get_random_batch(
        self, training: bool = True, batch_size: int = None
    ) -> tuple:
        """
        ...
        if len(images.shape) == 3:
            # Selected single image, which leads to collapse of first dim -->
            # must add dim back
    ...
    def batch_generator(
        ...
        while start_position < no_elements:
            ...
            yield (
                images[start_position:end_position],
                labels[start_position:end_position],
            )
            start_position = end_position

    def plot_example(
        self, images: np.ndarray = None, labels: np.ndarray = None
    ) -> None:
        """
